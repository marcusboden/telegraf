...
func (l *Libvirt) Gather(acc telegraf.Accumulator) error {
    ...
    domStat, err := connection.GetAllDomainStats(
      []*lv.Domain{&domain},
    ...
    domStats, err := connection.GetAllDomainStats(
      []*lv.Domain{&domain},
...
func GatherInterfaces(d lv.Domain, acc telegraf.Accumulator , tags map[string]string) error {
  return nil
